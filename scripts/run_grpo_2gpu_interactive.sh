#!/bin/bash
# 2-GPU GRPO launcher for interactive srun session
# vLLM on GPU 0, tool server + training on GPU 1
# Usage: inside an srun session with 2 GPUs, run: bash this_script.sh
set -euo pipefail
...
echo "=== 2-GPU GRPO Training (Interactive) ==="
echo "Node: $(hostname)"
echo "GPUs:"
...
CUDA_VISIBLE_DEVICES=1 python scripts/run_grpo.py \
    ...
    2>&1 | tee "${LOG_DIR}/grpo_2gpu_$(date +%Y%m%d_%H%M%S).log"
